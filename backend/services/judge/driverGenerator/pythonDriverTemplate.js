...
def parse_raw_input_to_args(raw_input, param_names):
    if isinstance(raw_input, dict):
        ...
        generic_aliases = ['nums', 'arr', 'array', 'values', 'list', 'items', 'elements', 'target', 'k', 'n', 'matrix', 'grid', 's', 'str']
        ...
        for idx, name in enumerate(param_names):
            ...
            if isinstance(val, str):
                try:
                    ...
                    pass
            if isinstance(val, list) and len(val) > 0 and isinstance(val[0], dict) and "val" in val[0]:
...
def __sarthi_run_driver__():
    ...
    if fn is None:
        ...
        user_fns = [obj for name, obj in globals().items() if callable(obj) and not name.startswith("__") and name not in ['parse_list_node', 'parse_tree_node', 'serialize_output', 'parse_raw_input_to_args', '__sarthi_run_driver__', 'ListNode', 'TreeNode', 'List', 'Optional']]
        if user_fns:
            fn = user_fns[0]
    ...
    for idx, tc in enumerate(test_cases):
        ...
        try:
            ...
            serialized_res = serialize_output(res)
