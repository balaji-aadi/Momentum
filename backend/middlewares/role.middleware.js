...
export const vendorOnly = asyncHandler(async (req, res, next) => {
    if (!req.user || !req.user._id) {
        return res.status(401).json(new ApiError(401, "Unauthorized User"));
    }
    ...
    try {
        ...
        if (userRole.role_id === 2 || userRole.name === 'vendor') {
            return next();
        } else {
            return res.status(401).json(new ApiError(401, "Unauthorized User"));
...
export const deliverymanOnly = asyncHandler(async (req, res, next) => {
    if (!req.user || !req.user._id) {
        return res.status(401).json({ error: 'Unauthorized User' });
    }
    ...
    try {
        ...
        if (userRole.role_id === 3 || userRole.name === 'deliveryman') {
            return next();
        } else {
            return res.status(401).json({ error: 'Unauthorized User' });
        }
...
export const customerOnly = asyncHandler(async (req, res, next) => {
    ...
    try {
        ...
        if (userRole.role_id === 4 || userRole.name === 'customer') {
            return next();
        } else {
            return res.status(401).json({ error: 'Unauthorized User' });
        }
